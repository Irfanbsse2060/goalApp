...
const setupUrlLogs = (req, res, next) => {  
  ...
  if (req.user) {
    if (!_.some(excludedFilesFromWinstonLogging, file => appUtils.stringEndsWith(url, file))) {
      console.log(appUtils.makeLogContextString(req) + '[' + req.method + ']: ' + req.hostname + req.url)
                  
      if (req.method === 'POST' && !_.some(excludePostBodyFromWinstonLogging, url => req.url === url))
